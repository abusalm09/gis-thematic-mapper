...
async function runMigrations() {
  ...
  try {
    ...
    // Drop tables in reverse order of dependencies to reset database
    try { await conn.execute('DROP TABLE IF EXISTS `activityLog`'); } catch (_) { }
    try { await conn.execute('DROP TABLE IF EXISTS `automationRules`'); } catch (_) { }
    try { await conn.execute('DROP TABLE IF EXISTS `generatedMaps`'); } catch (_) { }
    try { await conn.execute('DROP TABLE IF EXISTS `mapRequests`'); } catch (_) { }
    try { await conn.execute('DROP TABLE IF EXISTS `datasets`'); } catch (_) { }
    try { await conn.execute('DROP TABLE IF EXISTS `users`'); } catch (_) { }
